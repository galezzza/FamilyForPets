...
namespace FamilyForPets.Files.Contracts
{
    ...
    {
        Task<UnitResult<ErrorList>> CancelMultipartUpload(
            MultipartUploadCancelRequest request,
            CancellationToken cancellationToken = default);

        Task<Result<string, ErrorList>> CompleteMultipartUpload(
        ...
        Task<Result<string, ErrorList>> GetDownloadFileUrl(
            GetPresignedUrlToDownloadFullFileFromFileServiceRequest request,
            CancellationToken cancellationToken = default);

        Task<Result<MultipartUploadStartResponse, ErrorList>> GetStartUploadMultipartFileUrl(
            MultipartUploadStartRequest request,
            CancellationToken cancellationToken = default);

        Task<Result<GetPresignedUrlToUploadChunkOfFileToFileServiceResponse, ErrorList>> GetUploadChunkFileUrl(
            GetPresignedUrlToUploadChunkOfFileToFileServiceRequest request,
            CancellationToken cancellationToken = default);

        Task<Result<string, ErrorList>> GetUploadFullFileUrl(
